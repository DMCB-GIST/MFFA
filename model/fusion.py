...
class GloRe_Unit(nn.Module):
    def __init__(self, num_in, num_mid,
        ...
        self.gcn = GCN(num_feature=self.num_s, num_node=self.num_n)
        # extend dimension
        self.conv_extend = ConvNd(self.num_s, num_in, kernel_size=1, bias=False)

    def forward(self, x):
        n = x.size(0)
        x_w = self.conv_state(x).view(n, self.num_s, -1)
        x_v = self.conv_proj(x).view(n, self.num_n, -1)

        x_rproj_reshaped = torch.transpose(x_v, 1, 2)
        x_n_state = torch.bmm(x_v, torch.transpose(x_w, 1, 2))
        x_n_rel = self.gcn(x_n_state)
        x_state_reshaped = torch.matmul(x_rproj_reshaped, x_n_rel)
        x_state = x_state_reshaped.view(-1, self.num_s, 1)
